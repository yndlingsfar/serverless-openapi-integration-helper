class OptionsResolver {
    ...
    _mappingPath(stage) {
        ...
        _proxyManager(stage) {
            ...
            let getProxyManager = function (options, proxyTemplate) {
                if (!options.proxyManager.hasOwnProperty(proxyTemplate)) {
                    throw new TypeError(`There is no proxyManager template named ${proxyTemplate}`);
                }
                return options.proxyManager[proxyTemplate];
            }
            ...
            if (options.mapping) {
                options.mapping.forEach(function (mapping) {
                    if (Array.isArray(mapping.stage)) {
                        // Integration mapping stage array syntax
                        if (mapping.stage.includes(stage) && mapping.hasOwnProperty('proxyTemplate')) {
                            proxy = getProxyManager(options, mapping.proxyTemplate);
                        }
                    } else {
                        if (mapping.stage === stage) {
                            proxy = getProxyManager(options, mapping.proxyTemplate);
        ...
        if (proxy === null && options.autoMock) {
            //Todo
        }
    ...
    _validate() {
        let allowedConfigurations = ['package', 'proxyManager', 'inputFile', 'inputDirectory', 'outputDirectory', 'outputFile', 'mapping', 'cors', 'autoMock', 'validation'];
        Object.entries(this.options).forEach(([key, value]) => {
            if (!allowedConfigurations.includes(key)){
                throw new Error(`unrecognized option ${key}. 
            Allowed options are package, proxyManager, inputFile, inputDirectory, outputDirectory, outputFile, mapping, cors, autoMock, validation`);
            }
        })
    }
    ...
    resolve(stage = "dev") {
        ...
        return {
            ...
            proxyManager: this._proxyManager(stage),
        }
    }
}
